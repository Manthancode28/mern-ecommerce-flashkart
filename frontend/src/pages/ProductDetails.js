...
import { useSquad } from '../context/SquadContext'; // NEW: Import Squad Hook
...
const ProductDetails = () => {
  ...
  const { socket, squadId } = useSquad() // NEW: Get Socket and Squad ID
  ...
  // ----------------------------------------------------
  // SQUAD MODE: REAL-TIME SYNC LOGIC
  // ----------------------------------------------------
  useEffect(() => {
    fetchProductDetails()

    if (socket && squadId) {
      // 1. Emit: Tell your friend you are viewing this specific product
      socket.emit("sync_product_view", { 
        squadId, 
        productId: params?.id, 
        productName: data?.productName 
      });

      // 2. Listen: If friend navigates to a product, you follow them
      socket.on("receive_product_sync", (syncData) => {
        if (syncData.productId !== params?.id) {
          toast.info(`Partner is viewing: ${syncData.productName}`);
          navigate(`/product/${syncData.productId}`);
        }
      });
    }
    ...
    return () => {
      socket?.off("receive_product_sync");
    }
  }, [params?.id, socket, squadId]);
  // ----------------------------------------------------
